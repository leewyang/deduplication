...
This is an alternative version of ray_data_dedup_b75d209.py that replaces
groupby().map_groups() with groupby().aggregate() using custom AggregateFnV2
functions, followed by flat_map to explode results back into rows.

Architecture:
1. MinHash signature generation (map_batches)
2. LSH banding to generate candidate pairs (flatmap + aggregate)
3. Connected components to find duplicate clusters (iterative aggregate-reduce)
"""
...
class EdgesFromCollisionsAgg(AggregateFnV2[List, List]):
    ...
    def finalize(self, acc: List) -> List:
        ...
        return [(min_id, did) for did in unique_ids]
...
def distinct_2col(
    ...
    logger.info("distinct_2col using aggregate...")
