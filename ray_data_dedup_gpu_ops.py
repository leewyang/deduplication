...
def distinct_2col(
    ...
    if num_gpus > 0:
        ...
        def distinct_map_groups_gpu(batch: cudf.DataFrame) -> cudf.DataFrame:
            ...
            return unique_df
        current_ds = (
            current_ds.select_columns([col_1, col_2])
            .groupby([col_1], num_partitions=int(parallelism / 10))
            .map_groups(distinct_map_groups_gpu, batch_format="cudf")
            .materialize()
        )
    else:
    ...
    for i in range(max_iterations):
        ...
        if num_gpus > 0:
            ...
            current_ds = (
                ...
                .map_groups(large_star_map_groups_gpu, batch_format="cudf")
                .materialize()
            )
        ...
        if num_gpus > 0:
            ...
            current_ds = (
                current_ds
                .groupby(['node'], num_partitions=int(parallelism/10))
                .map_groups(small_star_map_groups_gpu, batch_format="cudf")
                .materialize()
            )
    ...
    if edges_checkpoint_uri is not None and check_path_exists(edges_checkpoint_uri):
        ...
        if num_gpus > 0:
            ...
            edges_ds = (
                ...
                .map_groups(create_edges_from_collisions_gpu_block, batch_format="cudf")
                .materialize()
                # bands_ds
                # .repartition(keys=['band_id', 'band_hash'], num_blocks=int(hash_parallelism/10))
                # .map_batches(create_edges_from_collisions_gpu_block, batch_format="cudf", num_gpus=1, batch_size="block")
                # .materialize()
            )
        else:
