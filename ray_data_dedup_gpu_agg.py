"""
Large-scale deduplication using Ray Data with groupby().aggregate() operations.

Based on the approach from: https://huggingface.co/blog/dedup

This implementation replaces groupby().map_groups() with groupby().aggregate()
for all group reduction steps, using AsList + flat_map for multi-row expansions.

Architecture:
1. MinHash signature generation (map_batches)
2. LSH banding to generate candidate pairs (flatmap + groupby + aggregate + flat_map)
...
import pandas as pd
import ray
from ray.data.aggregate import AsList, Count
...
def expand_band_to_edges(row: dict) -> list:
    """Expand a collected doc_ids list into (min_id, doc_id) star edges.

    This is the flat_map counterpart to the aggregate(AsList) step that
    replaced groupby().map_groups(create_edges_from_collisions).
    """
    doc_ids = row['doc_ids']
    if len(doc_ids) <= 1:
        return []
    min_id = min(doc_ids)
    return [{'src': min_id, 'dst': d} for d in doc_ids]


def band_to_edges_cudf_batch(batch) -> "cudf.DataFrame":
    """Vectorized GPU version of expand_band_to_edges.

    Input columns: band_id, band_hash, doc_ids (list<int64>)
    Output columns: src (int64), dst (int64)

    Replaces flat_map(expand_band_to_edges) for the GPU path.
    One map_batches call per partition instead of one flat_map call per row.
    """
    import cudf

    if not isinstance(batch, cudf.DataFrame):
        batch = cudf.DataFrame.from_pandas(batch)

    batch = batch.reset_index(drop=True)
    batch["_row"] = cudf.Series(range(len(batch)), dtype="int64")

    exploded = (
        batch[["_row", "doc_ids"]]
        .explode("doc_ids")
        .rename(columns={"doc_ids": "doc_id"})
    )
    exploded = exploded.dropna(subset=["doc_id"])

    if len(exploded) == 0:
        return cudf.DataFrame(
            {"src": cudf.Series([], dtype="int64"), "dst": cudf.Series([], dtype="int64")}
        )

    min_ids = exploded.groupby("_row")["doc_id"].min().reset_index()
    min_ids.columns = ["_row", "min_id"]
    counts = exploded.groupby("_row")["doc_id"].count().reset_index()
    counts.columns = ["_row", "cnt"]

    valid = counts[counts["cnt"] > 1][["_row"]].merge(min_ids, on="_row")
    if len(valid) == 0:
        return cudf.DataFrame(
            {"src": cudf.Series([], dtype="int64"), "dst": cudf.Series([], dtype="int64")}
        )

    edges_raw = exploded.merge(valid, on="_row")
    edges = edges_raw[edges_raw["doc_id"] != edges_raw["min_id"]][["min_id", "doc_id"]]
    edges.columns = ["src", "dst"]
    return edges


def large_star_cudf_batch(batch) -> "cudf.DataFrame":
    """Vectorized GPU large-star step on a batch with list-column 'parents'.

    Input columns: node (int64), parents (list<int64>)
    Output columns: node (int64), parent (int64)

    Processes all nodes in the batch with a single groupby+merge pass instead
    of one Python function call per node (as flat_map(expand_large_star) does).
    """
    import cudf
    ...
    # Explode list column to (node, parent) pairs
    exploded = (
        batch[["node", "parents"]]
        .explode("parents")
        .rename(columns={"parents": "parent"})
    )
    exploded = exploded.dropna(subset=["parent"])
    # keep parent dtype as-is (may be int64 or large_string)

    # Add self-loops: each node is its own neighbor
    nodes = batch[["node"]].drop_duplicates().reset_index(drop=True)
    self_loops = cudf.DataFrame({"node": nodes["node"], "parent": nodes["node"]})
    all_edges = cudf.concat([exploded[["node", "parent"]], self_loops]).drop_duplicates()

    if len(all_edges) == 0:
        return cudf.DataFrame(
            {"node": cudf.Series([], dtype="int64"), "parent": cudf.Series([], dtype="int64")}
        )

    # mp(u) = minimum neighbor for each node
    node_mp = all_edges.groupby("node")["parent"].min().reset_index()
    node_mp.columns = ["node", "mp"]

    # Emit (v, mp(u)) for each neighbor v > u
    large_edges = all_edges[all_edges["parent"] > all_edges["node"]]
    if len(large_edges) == 0:
        return cudf.DataFrame(
            {"node": cudf.Series([], dtype="int64"), "parent": cudf.Series([], dtype="int64")}
        )
    large_edges = large_edges.merge(node_mp, on="node")
    return cudf.DataFrame(
        {
            "node": large_edges["parent"].reset_index(drop=True),
            "parent": large_edges["mp"].reset_index(drop=True),
        }
    )


def small_star_cudf_batch(batch) -> "cudf.DataFrame":
    """Vectorized GPU small-star step on a batch with list-column 'parents'.

    Input columns: node (int64), parents (list<int64>)
    Output columns: node (int64), parent (int64)

    Processes all nodes in the batch with a single groupby+merge pass.
    """
    import cudf
    ...
    # Consider only small neighbors (parent <= node)
    small_edges = all_edges[all_edges["parent"] <= all_edges["node"]]
    if len(small_edges) == 0:
        return cudf.DataFrame(
            {"node": cudf.Series([], dtype="int64"), "parent": cudf.Series([], dtype="int64")}
        )

    # mp(u) = minimum small neighbor for each node
    node_mp = small_edges.groupby("node")["parent"].min().reset_index()
    node_mp.columns = ["node", "mp"]

    # Emit (w, mp(u)) for each small neighbor w <= u
    small_edges = small_edges.merge(node_mp, on="node")
    return cudf.DataFrame(
        {
            "node": small_edges["parent"].reset_index(drop=True),
            "parent": small_edges["mp"].reset_index(drop=True),
        }
    )
...
def distinct_2col(
    ...
    """Deduplicate (col_1, col_2) pairs using groupby+aggregate.

    Groups by both columns so each unique pair naturally produces one row;
    the Count() result column is dropped. Unified GPU+CPU path.
    """
    logger.info("distinct_2col using aggregate (unified GPU+CPU)...")
    current_ds = (
        current_ds
        .select_columns([col_1, col_2])
        .groupby([col_1, col_2], num_partitions=parallelism)
        .aggregate(Count())
        .select_columns([col_1, col_2])
        .materialize()
    )
...
def expand_large_star(row: dict) -> list:
    """Expand aggregated parents list for the large-star step.

    For each node group: find mp = min(parents ∪ {node}), then emit
    (v, mp) for all v in the neighborhood where v > node.

    Replaces large_star_map_groups_gpu / large_star_map_groups.
    """
    node = row['node']
    parents = list(set(row['parents']))          # deduplicate
    all_neighbors = list(set(parents + [node]))  # include node itself
    mp = min(all_neighbors)
    large_neighbors = [v for v in all_neighbors if v > node]
    return [{'node': v, 'parent': mp} for v in large_neighbors]


def expand_small_star(row: dict) -> list:
    """Expand aggregated parents list for the small-star step.

    For each node group: ensure identity neighbor, find mp = min(small
    neighbors), then emit (v, mp) for all v ≤ node.

    Replaces small_star_map_groups_gpu / small_star_map_groups.
    """
    node = row['node']
    parents = list(set(row['parents']))  # deduplicate
    if node not in parents:
        parents.append(node)             # ensure identity neighbor
    small_neighbors = [v for v in parents if v <= node]
    if not small_neighbors:
        return []
    mp = min(small_neighbors)
    return [{'node': v, 'parent': mp} for v in small_neighbors]
...
def compute_connected_components_distributed(
    ...
    1. Large-star: For each edge (u,v), point the larger node to the smaller
        ...
        Group by node, collect parents with AsList, expand via flat_map:
    ...
    for i in range(max_iterations):
        ...
        # Step 1: Large-star
        if num_gpus > 0:
            # GPU emit, aggregate into list-per-node, then expand via vectorized cuDF batch
            ...
            current_ds = (
                ...
                .aggregate(AsList(on='parent', alias_name='parents'))
                .map_batches(large_star_cudf_batch, batch_format="cudf", num_gpus=1, batch_size=100_000)
                .materialize()
            )
            logger.info("Length of large_star_expand_gpu: %s", current_ds.count())
        else:
            # CPU
            ...
            current_ds = (
                ...
                .aggregate(AsList(on='parent', alias_name='parents'))
                .flat_map(expand_large_star)
                .materialize()
            )
            logger.info("Length of large_star_expand: %s", current_ds.count())
        if verbose:
        ...
        if num_gpus > 0:
            ...
            current_ds = (
                ...
                .groupby(['node'], num_partitions=int(parallelism/10))
                .aggregate(AsList(on='parent', alias_name='parents'))
                .map_batches(small_star_cudf_batch, batch_format="cudf", num_gpus=1, batch_size=100_000)
                .materialize()
            )
            logger.info("Length of small_star_expand_gpu: %s", current_ds.count())
        else:
            # CPU
            ...
            current_ds = (
                ...
                .groupby(['node'], num_partitions=parallelism)
                .aggregate(AsList(on='parent', alias_name='parents'))
                .flat_map(expand_small_star)
                .materialize()
            )
            logger.info("Length of small_star_expand: %s", current_ds.count())
...
def find_duplicate_components(
    ...
    if edges_checkpoint_uri is not None and check_path_exists(edges_checkpoint_uri):
        ...
        _agg_ds = (
            ...
            .aggregate(AsList(on='doc_id', alias_name='doc_ids'))
        )
        if num_gpus > 0:
            edges_ds = _agg_ds.map_batches(
                band_to_edges_cudf_batch, batch_format="cudf", num_gpus=1, batch_size=100_000
            ).materialize()
        else:
            edges_ds = _agg_ds.flat_map(expand_band_to_edges).materialize()
